...
def motion_detection():
    ...
    while True:
        ...
        for c in cnt[-2]:
            ...
            if cv2.contourArea(c) > 5000: # if contour area is less then 5000 non-zero(not-black) pixels(white)
                ...
                text = 'Detected'
                # text that appears when there is motion in video feed and sound an alarm
                playsound('alarm.mp3')
                time.sleep(10)
            else:
                pass
        ...
        cv2.putText(frame, f'[+] Motion {text}', (10,20), font, 0.5,(0, 0, 255), 2)
